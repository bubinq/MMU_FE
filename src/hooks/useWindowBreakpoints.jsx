...
export default function useWindowBreakpoints(breakpoints) {
  ...
  useEffect(() => {
    const mobileQuery = window.matchMedia(`screen and (max-width: ${breakpoints.tablet - 0.1}px)`);
    const tabletQuery = window.matchMedia(
      `screen and (min-width: ${breakpoints.tablet}px) and (max-width: ${breakpoints.desktop - 0.1}px)`
    );
